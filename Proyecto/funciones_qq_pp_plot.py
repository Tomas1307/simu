...
from scipy.stats import expon, norm, lognorm, gamma, weibull_min, beta, uniform, chi2, triang, truncnorm
...
def PP_QQ_plot_truncated_normal(data, media="estimado", desvesta="estimado"):
    ...
    n = len(data)
    # Se crea una Q-Q plot con el paquete statsmodels.api para la distribución normal truncada
    sm.qqplot(data, truncnorm, distargs=(0, np.inf), loc=mean, scale=std_dev, line='45')
    ...
    pp = np.sort(truncnorm.cdf(data, 0, np.inf, loc=mean, scale=std_dev))
